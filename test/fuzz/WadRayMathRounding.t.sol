...
contract WadRayMathRoundingTests is Test {
    ...
    function test_rayMulFloor_exampleValues() public {
        ...
        assertEq(w.rayMulFloor(a, b), product / RAY);
        assertEq(w.rayMulFloor(0, b), 0);
        assertEq(w.rayMulFloor(a, 0), 0);

        uint256 tooLargeA = type(uint256).max / b + 1;

        vm.expectRevert();
        w.rayMulFloor(tooLargeA, b);
    }
    ...
    function test_rayMulCeil_exampleValues() public {
        ...
        assertEq(w.rayMulCeil(a, b),     expected);
        assertEq(w.rayMulCeil(RAY, RAY), RAY);
        assertEq(w.rayMulCeil(0, b),     0);
        assertEq(w.rayMulCeil(a, 0),     0);

        uint256 tooLargeA = type(uint256).max / b + 1;
        vm.expectRevert();
        w.rayMulCeil(tooLargeA, b);
    }
    ...
    function test_rayDivCeil_exampleValues() public {
        ...
        assertEq(w.rayDivCeil(a, b), expected);
        assertEq(w.rayDivCeil(RAY, RAY), RAY);
        assertEq(w.rayDivCeil(0, b), 0);

        uint256 tooLargeA = type(uint256).max / RAY + 1;

        vm.expectRevert();
        w.rayDivCeil(tooLargeA, b);
